...
for i in tqdm(indices):
    for T in T_list:
        for P in P_list:
            ...
            data_path = f'goodreads_completions_{args.mode}_{args.model}-chat_500_temp-{T}_p-{P}_k-50-{i}_merged.json' \
                if args.model != 'gpt-3.5-instruct' \
                    else f'goodreads_completions_{args.mode}_{args.model}-chat_500_temp-{T}-p-{P}-{i}_merged.json'
                    
            if not osp.exists(data_path):
                print('-------------------------missing\n-----', data_path)
                continue

            output_path = f'perplexity_goodreads_completions_{args.mode}_{args.model}-chat_500_temp-{T}_p-{P}_k-50-{i}_merged.npy' \
                if args.model != 'gpt-3.5-instruct' \
                    else f'perplexity_goodreads_completions_{args.mode}_{args.model}-chat_500_temp-{T}-p-{P}-{i}_merged.npy'
