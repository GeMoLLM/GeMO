...
T_list = np.array([0.5, 0.8, 1.0, 1.2, 1.5])
P_list = np.array([0.90, 0.95, 0.98, 1.00])
...
def plot_stacked_barchart(str_list, data_list, filename, x_list=[]):
    hist_list = []
    ...
    for x in x_list:
        ax.plot(x, 0.8, "x", markersize=30, color='black')
...
for pi, P in enumerate(P_list):
    str_list = ['src'] + ["$T=" + str(T) + "$\n$p=" + str(P)+"$\n" for T in T_list[indices_T]]
    data_list = [sent_x_src]
    for i in indices_T:
        data_list.append([x for x in sent_x_gen[:,i,pi] if x != -1])
    plot_stacked_barchart(
        str_list, 
        data_list,
        f'fig_sentiment_stacked_barchart_{args.metric}_{args.mode}_{args.model}_T-[0.5,0.8,1.2,1.5]_P-{P}.pdf',
        x_list=x_list_overall[P][args.model][args.mode])
