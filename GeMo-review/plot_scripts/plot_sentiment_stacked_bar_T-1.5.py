...
def plot_stacked_barchart(str_list, data_list, filename):
    ...
    if args.mode == 'personalized':
        if args.model == 'llama-2-13b':
            ax.plot(4, 0.8, "x", markersize=30, color='black')
        elif args.model == 'vicuna-13b':
            for ii in range(1, 5):
                ax.plot(ii, 0.8, "x", markersize=30, color='black')
        else:
            raise NotImplementedError(f"Model {args.model} not implemented!")
    elif args.mode == 'personation':
        if args.model in ['llama-2-13b', 'vicuna-13b']:
            for ii in range(2, 5):
                ax.plot(ii, 0.8, "x", markersize=30, color='black')
        else:
            raise NotImplementedError(f"Model {args.model} not implemented!")
...
for ti, T in enumerate(T_list):
    ...
    for i in range(4):
        data_list.append([x for x in sent_x_gen[:,-1,i] if x != -1])
