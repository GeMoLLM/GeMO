...
sent_gen = pickle.load(
    open(osp.join(folder, f'summary_sentiment_goodreads_decay_{args.mode}_{args.model}-chat_500.pkl'), 'rb')) \
    if args.end_temperature == 1.0 or args.period == 20 \
        else pickle.load(open(osp.join(folder, f'summary_sentiment_goodreads_decay-{args.end_temperature}-{args.period}_{args.mode}_{args.model}-chat_500.pkl'), 'rb'))
...
def plot_stacked_barchart(str_list, data_list, filename):
    ...
    bar_positions = np.arange(len(str_list))
    ...
    for i in range(len(bins) - 1):
        ...
        if i == 0:
            bottoms = np.zeros(len(str_list))
        else:
            bottoms += bar_heights_previous
        # Plot the bars
        ax.bar(bar_positions, bar_heights, bar_width, bottom=bottoms, label=f'({bins[i]:.1f},{bins[i+1]:.1f})', color=shades[i][:len(bar_positions)])
...
for di, D in enumerate(decay_list):
    str_list = ['src'] + [D + "\n$p=" + str(P)+"$\n" for P in P_list]
    data_list = [sent_x_src] + [sent_x_gen[:,di,i] for i in range(4)]
    out_file = f'fig_sentiment_decay_stacked_barchart_{args.metric}_{args.mode}_{args.model}_decay-{D}.pdf' \
        if args.end_temperature == 1.0 and args.period == 20 \
            else f'fig_sentiment_decay_stacked_barchart_{args.metric}_{args.mode}_{args.model}_decay-{D}_{args.end_temperature}-{args.period}.pdf'
    plot_stacked_barchart(str_list, data_list, out_file)
    print(f'done plotting for decay = {D}!')
    
for pi, P in enumerate(P_list):
    str_list = ['src'] + [D + "\n$p=" + str(P)+"$\n" for D in decay_list]
    data_list = [sent_x_src] + [sent_x_gen[:,i,pi] for i in range(2)]
    out_file = f'fig_sentiment_decay_stacked_barchart_{args.metric}_{args.mode}_{args.model}_P-{P}.pdf' \
        if args.end_temperature == 1.0 and args.period == 20 \
            else f'fig_sentiment_decay_stacked_barchart_{args.metric}_{args.mode}_{args.model}_P-{P}_{args.end_temperature}-{args.period}.pdf'
    plot_stacked_barchart(str_list, data_list, out_file)
