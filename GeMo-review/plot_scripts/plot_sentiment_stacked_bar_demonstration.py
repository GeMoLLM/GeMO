...
# Example usage
input_color = ["#66c2a5", '#fc8d62']
shades = [get_shades(color)[::-1] for color in input_color]
shades = np.array(shades).T

shades = np.array([['#222222', '#444444', '#666666', '#888888', '#AAAAAA', '#CCCCCC'],
          ['#222222', '#444444', '#666666', '#888888', '#AAAAAA', '#CCCCCC']]).T
# print(shades, shades.shape)
...
def plot_stacked_barchart(str_list, data_list, filename):
    ...
    ax.set_xticklabels(str_list, fontsize=24)
    # ax.legend(loc='best')
    plt.savefig(osp.join(out_folder, filename), bbox_inches='tight')
    
# str_list = ['src'] + ["$T=" + str(T) + "$\n$p=" + str(P)+"$"]
str_list = ['src'] + ['gen']
data_list = [sent_x_src, sent_x_gen[:,1,0]]
plot_stacked_barchart(str_list, data_list, f'fig_demonstration_sentiment_stacked_barchart_{args.metric}_{args.mode}_{args.model}_T-{T}_P-{P}.pdf')
print(f'done plotting for T = {T}, P = {P}!')