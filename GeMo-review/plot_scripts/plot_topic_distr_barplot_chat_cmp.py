...
gen_path_1 = osp.join(folder, f'summary_topic_distr_goodreads_{args.mode}_llama-2-13b-chat_500.pkl')
gen_path_2 = osp.join(folder, f'summary_topic_distr_goodreads_{args.mode}_llama-2-13b-nonchat_500.pkl')
...
uni_gen, cnt_gen = data_gen_1[1.2][0.9]
cnt_gen_sel = get_count_indexed_topics(uni_src_sel, uni_gen, cnt_gen)
data_list.append([x/sum(cnt_gen) for x in cnt_gen_sel])

uni_gen, cnt_gen = data_gen_2[1.2][0.9]
cnt_gen_sel = get_count_indexed_topics(uni_src_sel, uni_gen, cnt_gen)
data_list.append([x/sum(cnt_gen) for x in cnt_gen_sel])

uni_gen, cnt_gen = data_gen_1[1.2][1.0]
cnt_gen_sel = get_count_indexed_topics(uni_src_sel, uni_gen, cnt_gen)
data_list.append([x/sum(cnt_gen) for x in cnt_gen_sel])

uni_gen, cnt_gen = data_gen_2[1.2][1.0]
cnt_gen_sel = get_count_indexed_topics(uni_src_sel, uni_gen, cnt_gen)
data_list.append([x/sum(cnt_gen) for x in cnt_gen_sel])
...
plot_grouped_barchart(
    str_list,
    data_list,
    f'fig_topic_distr_{args.mode}_chat_cmp.pdf',
