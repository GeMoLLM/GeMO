...
for ti, T in enumerate(T_list):
    for pi, P in enumerate(P_list):
        for i in range(args.n_gen):
            data_path = f'perplexity_goodreads_completions_{args.mode}_{args.model}-chat_500_temp-{T}-p-{P}-{i}_merged.npy'
            result[ti][pi][i] = np.load(osp.join(folder_path, data_path))
                
out_path = osp.join(folder_path, f'perplexity_goodreads_completions_{args.mode}_{args.model}-chat_500.npy')
print(out_path)
