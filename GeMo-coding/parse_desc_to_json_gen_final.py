...
for file_path, indices in zip(file_paths, train_idx):
    ...
    for i in indices:
        filename = f'description_{args.model}_{fileid}_{pid}_gpt-4_{args.input_id}_{i}.txt' \
            if 'claude' not in args.input_id \
                else f'description_{args.model}_{fileid}_{pid}_{args.input_id}_{i}.txt'
        with open(filename, 'r') as file:
            file_content = file.read()
            json_data = parse_file_to_json(file_content)
            json.dump(json_data, open(f'parsed_description_{args.model}_{fileid}_solution_{args.input_id}_{i}_{pid}.json', 'w'), indent=4)
